...
class ShiftSQLRepository(ShiftRepositoryInterface):
    ...
    def close_shift(self, shift_id: str) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE shifts SET status = 'closed' WHERE shift_id = ?",
                (shift_id,)
            )
    ...
    def get_x_report(self, shift_id: str) -> XReport:
        with sqlite3.connect(self.db_path) as conn:
            ...
            for product_id, total_quantity, total_price in cursor.fetchall():
                product_summary[product_id] = {"quantity": total_quantity, "total_price": total_price}

            products = [{"id": pid, "quantity": data["quantity"], "total_price": data["total_price"]}
                        for pid, data in product_summary.items()]

            return XReport(shift_id=shift_id, n_receipts=n_receipts, revenue=revenue, products=products)
    ...
    def get_lifetime_sales_report(self) -> SalesReport:
        with sqlite3.connect(self.db_path) as conn:
            ...
            for product_id, quantity, total_price in cursor.fetchall():
                product_summary[product_id] = {"quantity": quantity, "total_price": total_price}

            products = [{"id": pid, "quantity": data["quantity"], "total_price": data["total_price"]}
                        for pid, data in product_summary.items()]

            return SalesReport(total_receipts=total_receipts or 0, total_revenue=total_revenue or 0.0, products=products)
